...
namespace filesync::core::sync_data {
    ...
    Entry::Entry(const std::filesystem::path& path) :
        EntryBase{path},
        bufferForRemote{std::make_unique<buffer::FileBuffer>()},
        bufferForPrevious{std::make_unique<buffer::FileBuffer>()},
    ...
    void Entry::doSetBuffers(
        ...
        if (bufferTypeSupportsRemoteBuffer(bufferForRemote)) {
            this->bufferForRemote = 
                std::visit(buffer::visitor::CloneType{}, bufferForRemote);
        } else {
        ...
        if (bufferTypeSupportsPreviousBuffer(bufferForPrevious)) {
            this->bufferForPrevious = 
                std::visit(buffer::visitor::CloneType{}, bufferForPrevious);
        } else {
            throw data::Exception("This buffer type cannot be used for the buffer "\
    ...
    protocol::HandleOrFilePath Entry::getRemoteBufferHandle() {
        ...
        if (auto buffer = std::get_if<std::unique_ptr<buffer::FileBuffer>>(&bufferForRemote)) {
            return (*buffer)->getFilePath();
        } else if (auto buffer = 
            std::get_if<std::unique_ptr<buffer::ProtocolMemoryBuffer>>(&bufferForRemote)) { 
    ...
    bool Entry::bufferTypeSupportsRemoteBuffer(const buffer::Buffer& buffer) const {
        if (std::holds_alternative<std::unique_ptr<buffer::FileBuffer>>(buffer) ||
            std::holds_alternative<std::unique_ptr<buffer::ProtocolMemoryBuffer>>(buffer)) {
    ...
    bool Entry::bufferTypeSupportsPreviousBuffer(const buffer::Buffer& buffer) const {
        if (std::holds_alternative<std::unique_ptr<buffer::FileBuffer>>(buffer) ||
            std::holds_alternative<std::unique_ptr<buffer::CharArrayMemoryBuffer>>(buffer)) {
